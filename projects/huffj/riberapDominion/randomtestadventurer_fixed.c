...
void randomTestAdventurer(struct gameState *original_state, int player, int handPos) {
  ...
  int r;  
  r = playAdventurer(drawntreasure, &state_copy, currentPlayer, cardDrawn, temphand, handPos); 
  r_asserttrue(r, 0);  
...
int main(int argc, char *argv[]){
  ...
  for(n = 0; n < RANDOM_TEST_LOOPS; n++){
    ...
    // New to Assignment 5
    // Pick a random spot in the hand to hold Adventurer
    int handPos = floor(Random() * state.handCount[currentPlayer]);
    state.hand[currentPlayer][handPos] = 13;
  
    randomTestAdventurer(&state, currentPlayer, handPos);
  }
